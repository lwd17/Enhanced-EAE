...
if __name__ == "__main__":
    ...
    parser.add_argument('--wav2vec_path', nargs='+', required=True)
    ...
    dataloader1 = MultiAudio(args.data_path1, batch_size=args.batch_size, num_workers=8)
    dataloader2 = MultiAudio(args.data_path2, batch_size=args.batch_size, num_workers=8)
    ...
    mkdir(save_dir)
    G = Generator(hparams.dim_neck, hparams.speaker_embedding_size,768, hparams.freq, lr=1e-3, is_train=True,
                  loss_content=args.loss_content,
                  discriminator=args.dis,
                  lambda_gan=args.lambda_gan,
                  multigpu=args.multigpu,
                  lambda_wavenet=args.lambda_wavenet,
                  test_path=args.test_path,wav2vec_path=args.wav2vec_path[0],
                  args=args).to(device)

    G.optimize_parameters(dataloader1,dataloader2, args.epochs, device, experimentName=experimentName, save_dir=save_dir,
